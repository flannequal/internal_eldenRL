...
class MemoryClient:
    ...
    def __init__(self, process_name: str = "eldenring.exe", **kwargs):
        ...
        # Store AOB patterns from config
    ...
    def _scan_aob(self, aob_pattern_str: str) -> Optional[int]:
        ...
        try:
            ...
            preferred = {"gameassembly.dll", "eldenring.exe"}
            # add GameAssembly first if present
            for mod in all_mods:
        ...
        # Always try all modules as last resort (comment out if too slow)
        # for mod in all_mods:
        #     if mod not in modules_to_try:
        #         modules_to_try.append(mod)
    ...
    def attach(self) -> bool:
        ...
        try:
            ...
            for name, aob_pattern in self._aob_patterns.items():
                if (
                    name not in self._bases_static
                ):  # Only scan if not already a static address
                    resolved_addr = self._scan_aob(aob_pattern)
                    if resolved_addr:
                        self._bases_static[name] = (
                            resolved_addr  # Cache the resolved address
                        )

            # Check if essential bases are resolved immediately after attachment and AOB scans
            if not self._check_essential_addresses():
                logging.error(
                    "Failed to resolve essential base addresses after attachment. Detaching."
                )
                self.detach()
                return False
    ...
    def _check_essential_addresses(self) -> bool:
        ...
        for addr_key in critical_addresses:
            if self._resolve_pointer_path(addr_key) is None:
                logging.error(
                    f"Critical address '{addr_key}' could not be resolved.")
                return False
    ...
    def _read_memory(self, address: int, length: int, data_type: str = "int") -> Any:
        ...
        try:
            if data_type == "int":
                ...
                logging.warning(
                    f"Unsupported data type for memory read: {data_type}")
                return None
    ...
    def read_player_stamina(self) -> Optional[float]:
        ...
        try:
            ...
            if current_sp is None or max_sp is None or max_sp <= 0:
                return None
    ...
    def _write_last_grace(self, grace_id: int):
        ...
        try:
            ...
            logging.info(
                f"Set Last Grace to ID: {grace_id} at address {hex(addr)}")
        except Exception as e:
    ...
    def _execute_teleport(
        ...
        self._pm.write_bytes(
            coord_buffer, struct.pack("fffffI", x, y, z, cos, sin, map_id), 24
        )
    ...
    def warp(self, bonfire_name_or_id: Any) -> bool:
        ...
        if not lua_warp_aob_pattern:
            logging.error(
                "WarpFunction AOB pattern not found in config/addresses.yaml."
            )
            return False
        ...
        cs_lua_event_manager_addr = self._aob_patterns.get("CSLuaEventManager")
        if not cs_lua_event_manager_addr:
            logging.error("CSLuaEventManager address not resolved.")
            return False

        try:
            cs_lua_event_proxy = self._read_memory(
                cs_lua_event_manager_addr + 0x08, 8, "longlong"
            )
            cs_lua_event_script_imitation = self._read_memory(
                cs_lua_event_manager_addr + 0x18, 8, "longlong"
            )
        except Exception as e:
        ...
        self._pm.write_bytes(
            args_buffer, struct.pack("<Q", cs_lua_event_script_imitation), 8
        )
        self._pm.write_bytes(
            args_buffer + 8, struct.pack("<Q", cs_lua_event_proxy), 8)
        self._pm.write_bytes(
            args_buffer + 16, struct.pack("<Q", warp_id_arg), 8)
    ...
    def _resolve_boss_stats_address(self) -> None:
        if not self._pm or not self._boss_param_id:
            return

        base_addr = self._bases_static.get("WorldChrMan")
        if not base_addr:
            return
        wcm_ptr = self._read_memory(
            self._pm.base_address + base_addr, 8, "longlong")
        if not wcm_ptr:
            return

        try:
            begin = self._read_memory(
                wcm_ptr +
                self._wcm_offsets["character_list_begin_off"], 8, "longlong"
            )
            end = self._read_memory(
                wcm_ptr +
                self._wcm_offsets["character_list_end_off"], 8, "longlong"
            )
            if begin is None or end is None or end <= begin:
                return

            logging.info(
                f"Scanning for boss with Param ID {self._boss_param_id}...")
            p = begin
            count = 0
            while p < end:
                count += 1
                ...
                comp = self._read_memory(
                    ent + self._char_offsets["comp_190_off"], 8, "longlong"
                )
                if not comp:
                    continue

                stats = self._read_memory(
                    comp + self._char_offsets["stats_qword_off"], 8, "longlong"
                )
                if stats:
                    ...
                    self._boss_transform_addr = self._read_memory(
                        comp +
                        self._char_offsets["transform_68_off"], 8, "longlong"
                    )
                    return
            logging.warning(
                f"Finished scanning {count} entities. Boss not found.")
        except Exception as e:
            logging.error(f"Error while scanning for boss entity: {e}")

    def read_boss_position(self) -> Optional[Tuple[float, float, float]]:
        if not self._pm or not self._boss_transform_addr:
            return None
        try:
            addr = self._boss_transform_addr + \
                self._char_offsets["pos_xyz_off"]
