...
class EldenHybridEnv(gym.Env):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.MEMORY_DEBUG_INTERVAL = int(
            config.get("MEMORY_DEBUG_INTERVAL", 1))
        ...
        self.TIME_ALIVE_PENALTY_PER_SECOND = float(
            config.get("TIME_ALIVE_PENALTY_PER_SECOND", 1.0))
        # Penalty per second for not hitting the boss. Discourages stalling/running away.
        self.NO_BOSS_HIT_PENALTY_PER_SECOND = float(
            config.get("NO_BOSS_HIT_PENALTY_PER_SECOND", 5.0))
        self.PROGRESS_REWARD_SCALE = float(
            config.get("PROGRESS_REWARD_SCALE", 150.0))
        self.HEALING_FLASK_AMOUNT_HP_RATIO = float(config.get(
            "HEALING_FLASK_AMOUNT_HP_RATIO", 0.4))  # e.g. a flask heals 40% of max HP
        self.FLASK_USAGE_REWARD = float(config.get(
            "FLASK_USAGE_REWARD", 50.0))  # Reward for good flask use
        # Penalty for wasteful flask use
        self.FLASK_USAGE_PENALTY = float(
            config.get("FLASK_USAGE_PENALTY", -25.0))
        self.DODGE_SUCCESS_REWARD = float(config.get(
            "DODGE_SUCCESS_REWARD", 20.0))  # Reward for successful dodge
        # Penalty for dodging nothing (spamming)
        self.DODGE_WASTE_PENALTY = float(
            config.get("DODGE_WASTE_PENALTY", -5.0))
        # Bonus for hitting staggered boss
        self.STAGGER_ATTACK_BONUS = float(
            config.get("STAGGER_ATTACK_BONUS", 100.0))

        # Dodge spam detection parameters
        # Time window to consider dodges as spam
        self.DODGE_SPAM_THRESHOLD_S = float(
            config.get("DODGE_SPAM_THRESHOLD_S", 1.0))
        self.DODGE_SPAM_PENALTY = float(config.get(
            "DODGE_SPAM_PENALTY", -50.0))  # Penalty for spamming dodges
        ...
        self.NUMBER_DISCRETE_ACTIONS = int(
            config.get("NUMBER_DISCRETE_ACTIONS", 22))
        ...
        self.input = InputController(enabled=not bool(
            config.get("DISABLE_INPUT", False)))
        ...
        # This is now unused, but kept for potential future PvP mode
        ...
        # New flag for attack action (if needed for dodge logic)
        ...
        # Track start time for time_alive calculation
        self.prev_episode_start_time = time.time()
        # Track previous boss animation (unused in state, but kept for potential future use)
        self.prev_boss_animation_id = -1
    ...
    def _render_text_overlay(self, frame, hp, stam, boss_hp, dist):
        ...
        cv2.putText(frame, f"Player HP: {hp:.2f}", (10, 20), self.font,
                    self.font_scale, self.font_color, self.line_type)
        cv2.putText(frame, f"Stamina: {stam:.2f}", (10, 40), self.font,
                    self.font_scale, self.font_color, self.line_type)
        cv2.putText(frame, f"Boss HP: {boss_hp:.2f}", (10, 60),
                    self.font, self.font_scale, self.font_color, self.line_type)
        cv2.putText(frame, f"Distance: {dist:.2f}", (10, 80),
                    self.font, self.font_scale, self.font_color, self.line_type)
        return frame
    ...
    def _grab_screen_shot(self, hp=1.0, stam=1.0, boss_hp=1.0, dist=0.0) -> np.ndarray:
        ...
        if self.DEBUG_MODE:
            obs_with_overlay = self._render_text_overlay(
                obs.copy(), hp, stam, boss_hp, dist)
    ...
    def _read_state(self):
        ...
        # Use boss_hp if player_hp is valid
        boss_hp = self.game.boss_hp if hp is not None else 1.0
        dist = self.game.distance_to_boss or 0.0  # Use property directly
        # Use episode start time
        time_alive = max(0.0, time.time() - self.prev_episode_start_time)
        # How long since boss was last hit by player
        time_since_boss_dmg = max(0.0, time.time() - self.time_since_boss_dmg)
    ...
    def _compute_reward_and_termination(self, curr_hp: float, curr_stam: float, curr_boss_hp: float, first_step: bool,
        ...
        if first_step:
            # Initialize to avoid early penalty for first few seconds
        ...
        time_since_taken_dmg_reward = 25 if (
            time.time() - self.time_since_dmg_taken > 5) else 0
        ...
        if self.boss_death:
            ...
            no_hit_boss_penalty = - \
                (time_since_last_hit * self.NO_BOSS_HIT_PENALTY_PER_SECOND)
            no_hit_boss_penalty = max(
                no_hit_boss_penalty, -200)  # Cap the penalty
        ...
        if self.curr_boss_hp < 0.98:  # Check for *any* progress past initial full HP
            progress_reward = (1.0 - self.curr_boss_hp) * \
        ...
        time_alive_penalty = - \
            (time_alive * self.TIME_ALIVE_PENALTY_PER_SECOND)
        # Cap the penalty to prevent runaway negative rewards
        time_alive_penalty = max(time_alive_penalty, -300)
        ...
        if self.flask_used_this_step:  # This flag needs to be set in the step() method based on action
            ...
            # Assuming this property exists or can be added to EldenRingGame
            player_max_hp_val = self.game.player_max_hp_val
            if player_max_hp_val is not None:
                healing_threshold = player_max_hp_val * \
                    (1.0 - self.HEALING_FLASK_AMOUNT_HP_RATIO)
                # Check if current HP is below threshold to need a flask
                if self.curr_hp * player_max_hp_val < healing_threshold:
                    flask_reward = self.FLASK_USAGE_REWARD  # Reward for using it when needed
                else:
                    # Penalty for using it when not needed (e.g., almost full HP)
                    flask_reward = self.FLASK_USAGE_PENALTY
        ...
        if self.dodged_this_step:  # This flag needs to be set in the step() method based on action
            ...
            if time_since_last_dodge < self.DODGE_SPAM_THRESHOLD_S:
                ...
                logging.debug(
                    f"Dodge spam detected! Consecutive dodges: {self._consecutive_dodges}")
            else:
                ...
                logging.debug(
                    f"Well-timed dodge. Resetting consecutive dodges to 1.")

            self._last_dodge_time = current_time  # Update last dodge time

        # 7) Total reward calculation
        total_reward = hp_reward + time_since_taken_dmg_reward + \
            time_alive_penalty + flask_reward + dodge_reward

        total_reward += boss_dmg_reward + progress_reward + \
            no_hit_boss_penalty  # Removed stagger_attack_bonus
        ...
        # self.game_won is currently always False
    ...
    def step(self, action: int):
        ...
        hp, stam, boss_hp, dist, time_alive, time_since_boss_dmg, player_flask_count = self._read_state()
        ...
        if hp is None or stam is None or boss_hp is None:  # These are critical for basic function
            logging.error(
                "Failed to read critical game state. Terminating episode.")
        ...
        self.flask_used_this_step = (
            action == self.config.get("FLASK_ACTION_ID", -1))
        self.dodged_this_step = (
            action == self.config.get("DODGE_ACTION_ID", -1))
        self.attacked_this_step = (
            action == self.config.get("ATTACK_ACTION_ID", -1))
        ...
        if self.LOG_MEMORY_DEBUG and (self.step_iteration % max(1, self.MEMORY_DEBUG_INTERVAL) == 0):
            try:
                ...
                logging.info(
                    ...
                    # Added time_since_boss_dmg
                    f"time_alive={time_alive:.2f}s time_since_boss_dmg={time_since_boss_dmg:.2f}s"
                )
            except Exception as e:
                logging.info(
                    f"[MEM] step={self.step_iteration} hp={hp:.3f} stam={stam:.3f} boss_hp={boss_hp:.3f} "
                    # Added time_since_boss_dmg
                    f"pos=(N/A, N/A, N/A) dist={dist:.3f} time_alive={time_alive:.2f}s time_since_boss_dmg={time_since_boss_dmg:.2f}s (Error getting player pos: {e})"
                )

        terminated = bool(death or boss_death or duel_won)
        # Max episode duration, using episode start time
        truncated = bool((time.time() - self.prev_episode_start_time) > 600)

        action_name = "UNKNOWN"
        if not (terminated or truncated):
            action_name = self.input.take_action(
                int(action))  # Execute the action

        logging.info(
            f"Action: {action} -> {action_name} | Reward: {reward:.3f}")

        # Compose observation - include all new state variables
        state_vec = [hp, stam, boss_hp, time_alive, dist, time_since_boss_dmg,
                     player_flask_count]  # Reduced state vector size
        obs = {
            # Capture image with current stats
    ...
    def reset(self, *, seed: int | None = None, options: Dict[str, Any] | None = None):
        ...
        if self.game.boss_hp is not None and self.game.boss_hp <= 1.0:
            logging.info(
                f"Successfully read initial boss HP: {self.game.boss_hp:.3f}. Proceeding with reset.")
        else:
            logging.warning(
                f"Game state not yet resolved (boss HP: {self.game.boss_hp})")
            return {}, {}
        ...
        # Reset to NaN so first HP read triggers update
        self.prev_hp = float('nan')
        self.curr_hp = float('nan')
        self.prev_boss_hp = float('nan')
        self.curr_boss_hp = float('nan')
